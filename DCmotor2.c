...
unsigned long IN1;// input from PA2
...
int main(void){
  ...
  while(1){ 
		...
		IN1 = GPIO_PORTF_DATA_R&0x04;// READ PA2 into IN1
		
		
		if(IN1 == 0 )// PA2 IS PRESSED
		{
			GPIO_PORTA_DATA_R |= (0x02); // 1A =0 2A =1
			GPIO_PORTA_DATA_R &= ~(0x01);
		}
		else 
		{
			GPIO_PORTA_DATA_R |= (0x01); // 1A =1 2A =0
...
void Port_Init(void)
  ...
  GPIO_PORTA_AMSEL_R &= ~0x27;      // no analog 
  GPIO_PORTA_PCTL_R &= ~0x00F00FFF; // regular function
  GPIO_PORTA_DIR_R |= 0x23;     // make PA5 PA1 PA0 out
	GPIO_PORTA_DIR_R &= (~0x04); // PA2 in
  GPIO_PORTA_DR8R_R |= 0x20;    // can drive up to 8mA out
  GPIO_PORTA_AFSEL_R &= ~0x27;  // disable alt funct on PA5,2,1,0
  GPIO_PORTA_DEN_R |= 0x27;     // enable digital I/O on PA5,2,1,0
}
